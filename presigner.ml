...
let main region url (verb : [`Get | `Put]) duration gen_command =
...
let url =
  ...
  Arg.(required & pos 1 (some uri) None & info [] ~docv:"URL" ~doc)
...
let main_t = Term.(const Lwt_main.run $ (const main $ region $ url $ verb $ duration $ gen_command))
