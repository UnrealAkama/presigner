...
let get_bucket url =
  ...
  | Some b -> b
  | None -> raise (Incorrect_Bucket_Name "Need a bucket in the url.")
...
let process_body b default =
  ...
  | _ -> (match Str.search_forward region_regex b 0 with
    | exception Not_found -> raise (Incorrect_AWS_Error (sprintf "AWS is missing a region field, it returned:\n %s" b))
    | _ -> Str.matched_group 1 b)
...
let get_region ~credentials ~bucket () =
  ...
  let formatted_date = sprintf "%02.0f%02.0f%02.0fT%02.0f%02.0f%02.0fZ" (float_of_int y) (float_of_int m) (float_of_int d) (float_of_int h) (float_of_int mi) (float_of_int s) in
  let host = "s3.amazonaws.com" in
  let path = sprintf "/%s" bucket in
  let region = "us-east-1" in
  let headers = Headers.empty in
  let headers = List.fold_left (fun acc x -> Headers.add ~key:(fst x) ~value:(snd x) acc) headers
  [
    ("Host", host);
    ("Date", formatted_date)
  ] in
  let scope = Authorization.make_scope ~date ~region ~service in
  let signing_key = Authorization.make_signing_key ~date ~region ~service ~credentials () in
  let signature, signed_headers =
    Authorization.make_signature ~date ~time ~verb ~path ~headers ~query:[] ~signing_key ~scope ~payload_sha:empty_hash
  in
  let algo = "AWS4-HMAC-SHA256" in
  let creds = sprintf "%s/%s/%s/s3/aws4_request" credentials.Credentials.access_key date region in
  let auth_header = sprintf " %s Credential=%s, SignedHeaders=%s, Signature=%s" algo creds signed_headers signature in
  ...
  let%lwt _resp, body = Cohttp_lwt_unix.Client.get (Uri.make ~scheme ~host ~path ()) ~headers in
...
let main url (verb : [`Get | `Put]) duration gen_command region =
  ...
  let%lwt region = match region with
  | Some r -> Lwt.return (Region.of_string r)
  | None ->  get_region ~credentials ~bucket ()
  in
...
let region =
  let doc = "Tell the region in advance, this is mostly useful for bulk operations or speeding up operations." in
  Arg.(value & opt (some string) None & info ["r"; "region"] ~docv:"region" ~doc)

let gen_command =
  let doc = "Generates a mostly complete curl command for usage." in
  Arg.(value & flag & info ["c"; "command"] ~doc)

let main_t = Term.(const Lwt_main.run $ (const main $ url $ verb $ duration $ gen_command $ region))

let info =
  let doc = "Creates a simple link to a file for either uploading or downloading from S3 that is preauthenticated." in
  let man = [
    `S Manpage.s_bugs;
    `P "File an issue on the project issue tracker." ]
  in
  Term.info "presigner" ~version:"%%VERSION%%" ~doc ~exits:Term.default_exits ~man
